...
async function save_state_when_setup_done()
{

    while (!serial_text.includes('##SETUP DONE##'))
    {
...
emulator.add_listener('serial0-output-byte', function(byte)
{
    ...
    if(!booted && (serial_text.endsWith(':~# ') ))
    {
        serial_text = '';
        booted = true;
        emulator.serial0_send('sync;echo 3 >/proc/sys/vm/drop_caches;rmmod ne2k-pci;rmmod virtio-net; /root/run_at_startup_root.sh; echo 2323534554555020444f4e452323 | xxd -r -p;\n');
        save_state_when_setup_done(); 
    }
});
EOF
